...
pit_data_2022_raw = read.csv("PITSurvey_0925.csv", stringsAsFactors = FALSE)
...
# hoh_name = pit_data_2022 %>% count(init_hoh) %>% filter(n>1) %>% filter(!is.na(init_hoh) & !(init_hoh %in% c("", "8","9")))
# hoh_name = hoh_name[2:33,1]
# pit_data_2022 %>% filter(init_hoh %in% hoh_name) %>% filter(hoh_tonight != 0) %>% filter(hoh_pet != 0)
...
#the observed percentage and the projected percentage over three years are from people who answered the question about duration of homelessness.
...
# kbl(continuous_desc, booktabs = T) %>%
#   add_header_above(c(" ", "Observed" = 2, "Projected" = 2)) %>%
...
CrossTable(pit_data_2022$violence, pit_data_2022$GENDER, prop.r=FALSE, prop.c=FALSE,
